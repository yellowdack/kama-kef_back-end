...
class Mapper(SubMapperParent):
    ...
    def resource(self, member_name, collection_name, **kwargs):
        ...
        self.connect("formatted_" + name_prefix + collection_name, 
            # collection_path + ".:(format:json)", action='index', 
        ...
        for method, lst in new_methods.iteritems():
            ...
            for action in lst:
                ...
                if action != 'new':
                    name = action + "_" + name
                route_options['action'] = action
                # formatted_path = (action == 'new' and new_path + '.:(format:json)') or \
                #     "%s/%s.:(format:json)" % (new_path, action)
        ...
        for method, lst in member_methods.iteritems():
            ...
            for action in lst:
                ...
                #                                     member_name),
                #     "%s/%s.:(format:json)" % (member_path, action), **route_options)
                self.connect("%s%s_%s" % (name_prefix, action, member_name),
                    "%s/%s" % (member_path, action), **route_options)
            if primary:
                route_options['action'] = primary
                # self.connect("%s.:(format:json)" % member_path, **route_options)
        ...
        #              member_path + ".:(format:json)", **route_options)
